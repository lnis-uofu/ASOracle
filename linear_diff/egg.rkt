...
;; not locally defs.
(define (impl-exists? a) #'void)
(define (impl->operator a) #'void)
(define (representation-name a) #'void)
(define (operator-info a) #'void)
(define (real-operator-info a) #'void)
(define (operator-exists? a) #'void)
(define (reap a b c) #'void)
(define sow #'void)
(define (repr-conv? a) #'void)
(define *needed-reprs* #'void)
(define *node-limit* #'void)
(define *proof-max-length* #'void)
(struct rule (name input output itypes otype))
(define representation? #'void)
(define representation-type #'void)
(define types #'void)
(define (get-representation f) #'void)
(define (timeline-push! f) #'void)
(define (desugar-program f) #'void)
(define (exn:fail:user:herbie:missing? f) #'f)
(define *rules* '())
(define (sym-append f) #'f)
(define (context f) #'void)
(define (*context* f) #'f)
(define (make-debug-context f) '())
;; Flattens proofs
;; NOT FPCore format
